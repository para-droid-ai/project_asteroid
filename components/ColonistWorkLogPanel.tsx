...
export const ColonistWorkLogPanel: React.FC<ColonistWorkLogPanelProps> = ({ colonists, logs, tickCount }) => {
    ...
    return (
        <div className="flex flex-col bg-gray-800 rounded border-2 border-gray-500 px-3 py-2 h-48">
            ...
            <div className="flex-grow overflow-y-auto pr-2">
                {colonists.map((col, idx) =>
                    <div key={col.id} className="flex items-center mb-1">
                        ...
                        <span className="w-24 text-xs font-semibold">{col.id}</span>
                        <div className="flex-1 flex items-center bg-gray-900 rounded-sm overflow-hidden">
                            {[...Array(timelineLength).keys()].map(i => {
                                ...
                                const entry = logForColonist ? logForColonist[tick % logForColonist.length] : null;
